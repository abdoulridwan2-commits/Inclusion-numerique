...
def insight_card(text, label="💡 Interprétation"):
    """Affiche un encart d'interprétation sous un graphique/une carte."""
    st.markdown(
        f'<div class="insight-card"><b class="insight-label">{label}</b><br>{text}</div>',
        unsafe_allow_html=True,
    )
...
st.markdown(
    textwrap.dedent(
        """
    <div class="hero"><div class="hero-title">📡 Togo Digital Access</div><div class="hero-subtitle">Diagnostic territorial de l'accès aux services numériques au Togo</div><div class="hero-badge">🇹🇬 Économie numérique&nbsp;&nbsp;·&nbsp;&nbsp;Analyse territoriale&nbsp;&nbsp;·&nbsp;&nbsp;Aide à la décision</div></div><div class="hero-flag-ribbon"></div>
    """
    ).strip(),
    unsafe_allow_html=True,
)
...
with tab1:
    ...
    if agency_columns:
        ...
        fig_agencies = px.bar(
            ...
            color="Opérateur",
            color_discrete_sequence=["#006A4E", "#00543D", "#FFCE00", "#8a8a8a"],
        )
    ...
    # --------------------------------------------------------
    # INFRASTRUCTURES vs DENSITÉ DÉMOGRAPHIQUE (Objectif 3 du défi)
    # --------------------------------------------------------

    st.markdown(
        '<div class="section-title">'
        "📐 Infrastructures vs densité démographique"
        "</div>",
        unsafe_allow_html=True,
    )

    if "densite_worldpop_moyenne" in df.columns and "agences_telecom_total" in df.columns:

        fig_density_vs_agences = px.scatter(
            df,
            x="densite_worldpop_moyenne",
            y="agences_telecom_total",
            size="population_worldpop",
            color="niveau_priorite",
            hover_name="prefecture",
            hover_data=["population_worldpop", "agents_mobile_money"],
            title="Nombre d'agences télécom selon la densité de population par préfecture",
            labels={
                "densite_worldpop_moyenne": "Densité de population (hab/km²)",
                "agences_telecom_total": "Nombre d'agences télécom",
        ...
        fig_density_vs_agences.update_layout(
            height=480,
            plot_bgcolor="white",
            paper_bgcolor="white",
        )
        ...
        st.plotly_chart(
            fig_density_vs_agences,
            use_container_width=True,
        )

        correlation_density_agences = df["densite_worldpop_moyenne"].corr(
            df["agences_telecom_total"]
        )
        outliers = df[
            (df["densite_worldpop_moyenne"] > df["densite_worldpop_moyenne"].median())
            & (df["agences_telecom_total"] == 0)
        ]["prefecture"].tolist()
        insight_card(
            f"La corrélation entre densité de population et nombre d'agences télécom est de "
            f"<b>{correlation_density_agences:.2f}</b>. "
            + (
                f"<b>{len(outliers)}</b> préfecture(s) au-dessus de la densité médiane nationale "
                f"n'ont pourtant <b>aucune agence</b> télécom recensée : "
                f"{', '.join(outliers[:6])}{'…' if len(outliers) > 6 else ''} — ce sont les "
                f"écarts les plus visibles entre poids démographique et présence physique des "
                f"opérateurs."
                if outliers else
                "Aucune préfecture densément peuplée ne se retrouve totalement dépourvue d'agence."
            )
        )
    ...
    if (
        ...
        fig_operator = px.pie(
            ...
            color_discrete_sequence=["#006A4E", "#FFCE00", "#00543D", "#D21034", "#8a8a8a"],
        )
    ...
    # --------------------------------------------------------
    # INTERPRÉTATION (calculée dynamiquement depuis df)
    # --------------------------------------------------------
    top_pop_row = df.loc[df["population_worldpop"].idxmax()]
    low_mm_row = df.loc[df["agents_mm_pour_10000_hab"].idxmin()]
    top_operator = (
        operator_counts.iloc[0]["operateur"]
        if len(operator_counts) > 0 else "N/A"
    )
    top_operator_share = (
        operator_counts.iloc[0]["nombre"] / operator_counts["nombre"].sum() * 100
        if len(operator_counts) > 0 else 0
    )
    insight_card(
        f"<b>{top_pop_row['prefecture']}</b> est la préfecture la plus peuplée "
        f"({format_number(top_pop_row['population_worldpop'])} habitants estimés), tandis que "
        f"<b>{low_mm_row['prefecture']}</b> affiche la plus faible densité Mobile Money "
        f"({format_decimal(low_mm_row['agents_mm_pour_10000_hab'])} agents / 10 000 hab.) — "
        f"un signal de sous-desserte à confirmer. Le réseau Mobile Money reste concentré chez "
        f"<b>{top_operator}</b> ({top_operator_share:.0f}% des points recensés), loin devant les "
        f"agences physiques des opérateurs, beaucoup moins nombreuses et plus centralisées."
    )
...
with tab2:
    ...
    if prefectures is not None:
        ...
        # ----------------------------------------------------
        # INTERPRÉTATION (calculée depuis la sélection filtrée)
        # ----------------------------------------------------
        if len(map_filtered) > 0:
            dominant_niveau = (
                map_filtered["niveau_priorite"]
                .astype(str)
                .value_counts()
                .idxmax()
            )
            part_dominant = (
                map_filtered["niveau_priorite"].astype(str).value_counts().max()
                / len(map_filtered) * 100
            )
            insight_card(
                f"Sur les <b>{len(map_filtered)}</b> territoire(s) affiché(s), le niveau "
                f"<b>{dominant_niveau}</b> domine ({part_dominant:.0f}% de la sélection), "
                f"représentant <b>{format_number(map_filtered['population_worldpop'].sum())}</b> "
                f"habitants concernés. "
                + (
                    f"{len(zero_antenna)} territoire(s) sans antenne recensée dans cette "
                    f"sélection appellent une vérification terrain prioritaire."
                    if zero_antenna else
                    "Toutes les préfectures de cette sélection ont au moins une antenne "
                    "recensée dans OpenCelliD."
                )
            )
...
with tab3:
    ...
    # --------------------------------------------------------
    # INTERPRÉTATION (calculée depuis top_priorites / df_filtre)
    # --------------------------------------------------------
    leader = top_priorites.sort_values(indicateur, ascending=False).iloc[0]
    score_median_national = df["score_priorite"].median()
    ecart_vs_median = (
        (leader["score_priorite"] - score_median_national) / score_median_national * 100
        if score_median_national else 0
    )
    nb_forte = (df_filtre["niveau_priorite"] == "Forte").sum()
    insight_card(
        f"<b>{leader['prefecture']}</b> arrive en tête de ce classement sur l'indicateur "
        f"« {indicateur_label} ». Son score de priorité est supérieur de "
        f"<b>{ecart_vs_median:.0f}%</b> à la médiane nationale, ce qui en fait un territoire "
        f"à examiner en premier (voir l'onglet Recommandations pour le plan d'action détaillé). "
        f"Sur l'ensemble des territoires actuellement filtrés, <b>{nb_forte}</b> sont classés "
        f"en priorité 🔴 Forte."
    )
